...
RM4OrdersListController = OrdersListController.extend({
	...
	data: function() {
      return {
        orders: Orders.find({"modality.name": "RESONADOR 05"}, this.findOptions()),
		nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
      };
    }
  });

RM7OrdersListController = OrdersListController.extend({
  
  data: function() {
      return {
        orders: Orders.find({"modality.name": "RESONADOR 07"}, this.findOptions()),
    nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
      };
    }
  });
...
RM1OrdersListController = OrdersListController.extend({
	  ...
	  data: function() {
        return {
          orders: Orders.find({"modality.name": "RESONADOR 06"}, this.findOptions()),
		  nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
        };
      }
    });
RM2OrdersListController = OrdersListController.extend({
	  ...
	  data: function() {
	        return {
	          orders: Orders.find({"modality.name": "RESONADOR 03"}, this.findOptions()),
			  nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
	        };
	      }
	    });	
RM3OrdersListController = OrdersListController.extend({
	...
	data: function() {
		return {
			orders: Orders.find({"modality.name": "RESONADOR 04"}, this.findOptions()),
			nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
		};
	}
});	

CTOrdersListController = OrdersListController.extend({
	
	data: function() {
		return {
			orders: Orders.find({"modality.name": "TOMOGRAFO 01"}, this.findOptions()),
			nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
		};
	}
});		
...
Router.map(function() {
    ...
    this.route('rm7Orders', {
      path: '/rm7/:ordersLimit?',
      controller: RM7OrdersListController
    });
    ...
    this.route('ctOrders', {
      path: '/ct/:ordersLimit?',
      controller: CTOrdersListController 
    });
