...
RM4OrdersListController = OrdersListController.extend({
	...
	data: function() {
      return {
        orders: Orders.find({"modality.name": "Resonador 5"}, this.findOptions()),
		nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
      };
    }
  });
...
RM7OrdersListController = OrdersListController.extend({
  ...
  data: function() {
      return {
        orders: Orders.find({"modality.name": "Resonador 7"}, this.findOptions()),
    nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
      };
    }
  });
...
RM1OrdersListController = OrdersListController.extend({
	  ...
	  data: function() {
        return {
          orders: Orders.find({"modality.name": "Resonador 6"}, this.findOptions()),
		  nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
        };
      }
    });
RM2OrdersListController = OrdersListController.extend({
	  ...
	  data: function() {
	        return {
	          orders: Orders.find({"modality.name": "Resonador 3"}, this.findOptions()),
			  nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
	        };
	      }
	    });	
RM3OrdersListController = OrdersListController.extend({
	...
	data: function() {
		return {
			orders: Orders.find({"modality.name": "Resonador 4"}, this.findOptions()),
			nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
		};
	}
});	
...
CTOrdersListController = OrdersListController.extend({
	...
	data: function() {
		return {
			orders: Orders.find({"modality.name": "Tomógrafo 1"}, this.findOptions()),
			nextPath: this.route.path({ordersLimit: this.ordersLimit() + this.increment})
		};
	}
});		
